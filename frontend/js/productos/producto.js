...
async function mostrarPRodcuto(){
    ...
    productos.forEach(ruta=>{
        const {_id, nombrePro, precio, marca, stock, descripcion, garantia, imagen} = ruta;
        carts.innerHTML += `
        <div class="cont-carts">
            <div class="imagen">
            <img class="hoalp deletee" width="auto" height="200px" src="${imagen}" alt="">
            </div>
            <div class="parrafos">
                <h1 class="title">${nombrePro}</h1>
            <h3 class="precio" style="color:white">Precio: ${precio}</h3>
            <h3>Marca: ${marca}</h3>
            <h3>${descripcion}</h3>
            <h3>Garantia: ${garantia}</h3>
                <button value="${_id}" data-id='${_id}' class='btn-add-cart sisen'>Detalles</button>
            </div>
            </div>
            `
    })
    ...
    function readTheContent(product){
        const infoProduct = {
            image: product.querySelector('.hoalp').src,
...
/* let imagen = document.querySelector('.hoalp');
let src = imagen.getAttribute('src');
console.log(src);  // Imprime el src de la imagen */
